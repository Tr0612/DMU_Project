...
class CustomSAC(SAC):
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            # 🚨 Changed line: sample (data, indices)
            replay_data, indices = self.replay_buffer.sample(batch_size)
            ...
            with torch.no_grad():
                ...
                target_q_values = replay_data.rewards + (1 - replay_data.dones) * self.gamma * (next_q_values - self.ent_coef * next_log_prob)
            ...
            # Actor update
            actions_pi, log_prob = self.actor.sample(replay_data.observations)
            q_pi = torch.minimum(
                self.critic.q_net1(replay_data.observations, actions_pi),
                self.critic.q_net2(replay_data.observations, actions_pi),
            )
